...
This property can be set as a unique identifier in the openVideoCreator() function. [We will send it back](/webhook.html) with the produced video so you can know where the video belongs to.   
*Sending videoContent is highly recommended*
***  
**showCreativeGallery** <code>boolean</code> <Badge text="Optional" /> *(default: true)*  
...
***
**showUploadGallery** <code>boolean</code> <Badge text="Optional" /> *(default: true)*   
Whether to show the tab that allows users to upload custom media 
***
**destLanguage** <code>string</code> <Badge text="Optional" />  *(default: en)*   
Pass language code to adjust the Video Editor UI. Available options are en, fr and es.
***
...
***
**categoryNames** <code>string[]</code> <Badge text="Optional" />  *(default: all available templates will be displayed)*  
...
::: danger Important!
Getting the produced video by listening to the <code>produce_done</code> event is less secured. It's also irrelevant if the user has closed the video creator. We recommand implementing a listener to [a webook endpoint](/webhook.html)
